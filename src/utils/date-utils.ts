export default class DateUtils {
	...
	static convertFromISOString(isoString: string): Date {
		return new Date(isoString);
	}

	static formatDateTimePtFromISOString(isoString: string): string {
		const date = this.convertFromISOString(isoString);
		return this.getDateTimePt(date);
	}
}
