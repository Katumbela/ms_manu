...
const studentInfo = {
	...
	classGroup: 'EINF-M2',
	institution: 'ISPTEC - Instituto Superior Politécnico de Tecnologias e Ciências',
	logo: 'ispt.svg',
	avatar: 'id.svg'
};

const studentDetails = [ { label: 'Turma', value: 'EINF-M1' }, { label: 'Sala', value: 'BAA3 - 2º andar' } ];
					...
					<div className={`${edu.tipo}`}>
						{studentStatus.map((status, index) => (
							<div key={index} className={edu.type}>
								...
								<p className={`${edu.bold} ${status.statusClass || ''}`}>{status.value}</p>
							</div>
						))}
					</div>
				</div>
